 ...
 * stereo.
 */
public class lab55 extends GLJPanel implements GLEventListener, KeyListener{
	 ...
	 */
	public void Trojkaty2(GL2 gl2,int iloscWieszcholkow){
        gl2.glClear(GL2.GL_COLOR_BUFFER_BIT);
        
        double kat = 0;
        gl2.glBegin(GL2.GL_POLYGON);
        double katZmiana = 2 * Math.PI / iloscWieszcholkow;
        for (int i = 1; i <= iloscWieszcholkow; i++) {
            gl2.glColor3f(x/0.6f,(x+z)*x/0.4f,z/0.5f);
            x = (float) (5 * Math.cos(kat));
            z = (float) (5 * Math.sin(kat));
            gl2.glVertex3f(x, 0.0f, z);
            kat = i * katZmiana;
        }
    ...
    public void Trojkaty1(GL2 gl2){
        ...
        for ( int i = 0; i<n;i++)
        {
            ...
            gl2.glVertex3f((float) (Math.cos((i+1)*2*Math.PI/n)*wielkosc), 0, (float) (Math.sin((i+1)*2*Math.PI/n)*wielkosc));
        }
        gl2.glEnd();
    }
    public void Ostroslup(GL2 gl2)
    {
    	Trojkaty2(gl2,8);
        Trojkaty1(gl2); 	
    }
	public void Korkociag(GL2 gl2,int maxPromien, int iloscObrotow){
        gl2.glClear(GL2.GL_COLOR_BUFFER_BIT);

        int iloscWieszcholkow = iloscObrotow*100;
        double katZmiana= iloscObrotow * Math.PI / iloscWieszcholkow;
        float wysokosc=0;
        float maxWysokosc=(iloscObrotow/800f)*iloscWieszcholkow;
        float maxProporcja=maxWysokosc/maxPromien;
        double promien=maxPromien;
        gl2.glBegin(GL2.GL_POLYGON);
        for (int i = 1; i <= iloscWieszcholkow; i++) {
        	promien=Math.pow(maxProporcja/(maxWysokosc-wysokosc), -1);
            kat =  i * katZmiana * Math.PI;
            gl2.glColor3f(x/0.3f,(x+z)*x/0.2f,z/0.25f);
            x = (float) (promien * Math.cos(kat));
            z = (float) (promien * Math.sin(kat));
            gl2.glVertex3f(x, wysokosc, z);
            wysokosc+=iloscObrotow/800f;
            
        }
        gl2.glEnd();
        gl2.glColor3f(255,165,0);
        gl2.glTranslated(0,0,-4.5);
        glu.gluCylinder(glu.gluNewQuadric(),1,1,9,100,50);
