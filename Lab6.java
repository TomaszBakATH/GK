...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	{
		float x,z,xNowe,zNowe;
		xNowe = 0;
		zNowe= 8;
		//ustawMaterial(gl2,5);
		for (int i = 1; i<= 8; i += 1) {
			gl2.glBegin(GL2.GL_TRIANGLES); 
			gl2.glVertex3f(xNowe, -1, zNowe);
			gl2.glMaterialfv (GL2.GL_FRONT, GL2.GL_AMBIENT_AND_DIFFUSE, materials[i+2], 1);
			x = (float) Math.sin(i * 2 * Math.PI / 8)*8;
			z = (float) Math.cos(i * 2 * Math.PI / 8)*8;
			xNowe=x;
			zNowe=z;
			gl2.glVertex3f(x, -1, z);
			gl2.glMaterialfv(GL2.GL_FRONT, GL2.GL_SPECULAR, materials[i-1],2);
			gl2.glVertex3f(0, 4, 0); 
			gl2.glMaterialfv(GL2.GL_FRONT, GL2.GL_SHININESS, materials[i+5],3);
			gl2.glEnd();
			}
	}
